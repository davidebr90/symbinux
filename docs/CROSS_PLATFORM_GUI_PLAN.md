...
> host-tool errors, and the theme menu persists to `settings.json`. Remaining
> Phase 1 work ports PBAP contacts, i18n, and notifications. The
...
| **1** | Port the GUI to `gtk4-rs` on **Linux** at feature parity linking the core directly. 🔨 *in progress:* `symbinux-gui` has the window shell, wordmark/version header, USB/Bluetooth/Wi-Fi selector, theme-aware empty-state logo, real USB detection with progress/cancel, capability-aware Nokia action buttons, a direct Identify card via `symbinux-transport`, Bluetooth/Wi-Fi scans via Linux host tools, and persisted theme selection. *Remaining:* PBAP contacts, i18n (11 langs), notifications | Rust GUI == current GUI, on Linux |
